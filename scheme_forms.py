...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        x = Pair(signature.first, expressions.first)
        y =  Pair(signature.rest, expressions.rest)
        env.define(signature.first, do_lambda_form(y, env))
...
def do_lambda_form(expressions, env):
    ...
    sig = expressions.first
    validate_formals(sig)
    # BEGIN PROBLEM 7
    exp = expressions.rest
    return LambdaProcedure(sig, exp, env)
    # END PROBLEM 7
